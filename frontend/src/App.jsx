...
/** Sidebar button that toggles the floating chat panel. */
function AssistantToggle() {
  const { isOpen, toggleChat } = useChat()
  return (
    <button
      onClick={toggleChat}
      className={`w-full flex items-start gap-3 px-3 py-2.5 rounded-lg transition-colors border-l-[3px] ${
        isOpen
    ...
    >
      ...
      <div className="min-w-0 text-left">
        <span className="text-sm font-medium text-gray-900 block">Assistant</span>
        <p className="text-xs text-gray-400 mt-0.5 leading-tight">Chat, ask questions, brain dump</p>
      </div>
    </button>
  )
}
...
function App() {
  ...
  return (
    ...
    <ToastProvider>
        ...
        <aside className="w-60 bg-white border-r border-gray-200 flex flex-col h-screen sticky top-0 flex-shrink-0">
          ...
          <nav className="flex-1 px-3 py-4 space-y-1">
            ...
            <SidebarLink
              ...
              label="Add to KB"
              description="Files, notes, transcripts"
            />
            <SidebarLink
              to="/audit"
              color="amber"
        ...
        <div className="flex-1 flex flex-col min-h-screen">
          ...
          <main className="max-w-5xl mx-auto px-6 py-8 w-full flex-1">
            <Routes>
              ...
              <Route path="/intake" element={<Navigate to="/ingest" replace />} />
              <Route path="/import" element={<Navigate to="/ingest" replace />} />
              <Route path="/process" element={<Navigate to="/ingest" replace />} />
            </Routes>
          </main>
      ...
      {/* Floating chat panel — rendered outside main layout so it overlays everything */}
      <Chat />
    </ToastProvider>
    </ChatProvider>
    </ProjectProvider>
